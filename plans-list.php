...
include "db-connection.php";
    ...
    if (isset($_POST['id']) && !empty($_POST['id'])) {
        $id = $_POST['id'];
        $del = "DELETE FROM plans  WHERE id='$id';";
        $delet = $conn->query($del);

        if ($delet) {
            header("Location: plans-list.php?success='Page deleted successfully!");
            die();
        } else {
            header("Location:plans-list.php?error='Unable to delete the page");
            die();
        }
    }
    ...
    $order_by = "plans." . $order_by;


    if (isset($_GET['s'])) {
        $s = $_GET['s'];

        $total_recordsResults = $conn->query("select count(*) as total_records from plans ;");
        $total_records = $total_recordsResults->fetch_assoc()['total_records'];

        $number_of_pages = ceil($total_records / $per_page);


     $sql =  $sql = "select * from plans order by $order_by $order limit $per_page OFFSET $offset_value;";


     } else {

        if ($_SESSION['role'] == 'admin') {
            $total_recordsResult = $conn->query("select count(*) as total_records from plans;");
        } else {
            $total_recordsResult = $conn->query("select count(*) as total_records from plans where author=" . $_SESSION['user_id'] . ";");
        }
        ...
        if ($_SESSION['role'] == 'admin') {
            $sql = "select * from plans order by $order_by $order limit $per_page OFFSET $offset_value;";
